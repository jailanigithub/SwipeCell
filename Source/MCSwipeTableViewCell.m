...
- (void)slideImageWithPercentage:(CGFloat)percentage imageName:(NSString *)imageName isDragging:(BOOL)isDragging {
    ...
    slidingImageRect = CGRectMake(position.x - slidingImageSize.width ,
            position.y - slidingImageSize.height,
            slidingImageSize.width,
                                  slidingImageSize.height + 20);
    ...
    if (_direction == MCSwipeTableViewCellDirectionRight) {
        ...
        position.x = 300 - slidingImageSize.width;
    }
    [_slidingImageView setFrame:CGRectMake(position.x, position.y - slidingImageSize.height, slidingImageSize.width, slidingImageSize.height)];
